...
__all__ = ["RequestContextMiddleware", "logging_middleware"]

async def logging_middleware(request: Request, call_next: Callable[[Request], Response]) -> Response:
    """
    Versão funcional do middleware para compatibilidade com os testes.
    Mantém o mesmo padrão de logging do RequestContextMiddleware.
    """
    req_id = request.headers.get("x-request-id") or new_request_id()
    structlog.contextvars.clear_contextvars()
    structlog.contextvars.bind_contextvars(request_id=req_id)
